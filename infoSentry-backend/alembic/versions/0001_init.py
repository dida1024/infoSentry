...
embedding_status_enum = sa.Enum(
    "pending", "done", "skipped_budget", "failed", name="embeddingstatus"
)
push_decision_enum = sa.Enum(
    "IMMEDIATE", "BATCH", "DIGEST", "IGNORE", name="pushdecision"
)
push_status_enum = sa.Enum(
    "PENDING", "SENT", "FAILED", "SKIPPED", "READ", name="pushstatus"
)
push_channel_enum = sa.Enum("EMAIL", "IN_APP", name="pushchannel")
feedback_type_enum = sa.Enum("LIKE", "DISLIKE", name="feedbacktype")
agent_trigger_enum = sa.Enum(
    "MatchComputed", "BatchWindowTick", "DigestTick", name="agenttrigger"
)
agent_run_status_enum = sa.Enum(
    "RUNNING", "SUCCESS", "TIMEOUT", "ERROR", "FALLBACK", name="agentrunstatus"
)
tool_call_status_enum = sa.Enum("SUCCESS", "ERROR", name="toolcallstatus")
action_type_enum = sa.Enum(
    "EMIT_DECISION", "ENQUEUE_EMAIL", "SUGGEST_TUNING", name="actiontype"
)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "timezone", sa.String(), nullable=False, server_default="Asia/Shanghai"
        ),
    )
    ...
    op.create_index(
        "ix_auth_magic_links_token", "auth_magic_links", ["token"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "fetch_interval_sec", sa.Integer(), nullable=False, server_default="1800"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.JSON(),
            nullable=False,
            server_default='["12:30", "18:30"]',
        ),
        sa.Column(
            "digest_send_time", sa.String(), nullable=False, server_default="09:00"
        ),
        sa.Column(
            "immediate_enabled",
            sa.Boolean(),
    ...
    op.create_index(
        "ix_goal_push_configs_goal_id", "goal_push_configs", ["goal_id"], unique=True
    )
    ...
    op.create_index(
        "ix_goal_priority_terms_goal_id", "goal_priority_terms", ["goal_id"]
    )
    ...
    op.create_unique_constraint(
        "uq_goal_item_matches_goal_item", "goal_item_matches", ["goal_id", "item_id"]
    )
    ...
    op.create_index(
        "ix_push_decisions_dedupe_key", "push_decisions", ["dedupe_key"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "input_snapshot_json", sa.JSON(), nullable=False, server_default="{}"
        ),
        sa.Column(
            "output_snapshot_json", sa.JSON(), nullable=False, server_default="{}"
        ),
        sa.Column("final_actions_json", sa.JSON(), nullable=False, server_default="[]"),
        sa.Column(
            "budget_snapshot_json", sa.JSON(), nullable=False, server_default="{}"
        ),
        sa.Column(
            "llm_used", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "embedding_tokens_est", sa.Integer(), nullable=False, server_default="0"
        ),
